// Taken from https://github.com/dart-lang/sdk/blob/965234ccbe07dde0bf9f13466928c56767054d8f/pkg/analyzer/lib/src/dart/resolver/exit_detector.dart
...
class ExitDetector extends GeneralizingAstVisitor<bool> {
  ...
  bool visitBinaryExpression(BinaryExpression node) {
    final lhsExpression = node.leftOperand;
    final rhsExpression = node.rightOperand;
    final operatorType = node.operator.type;
  ...
  bool visitConditionalExpression(ConditionalExpression node) {
    final conditionExpression = node.condition;
  ...
  bool visitDoStatement(DoStatement node) {
    final outerBreakValue = _enclosingBlockContainsBreak;
    final outerContinueValue = _enclosingBlockContainsContinue;
    _enclosingBlockContainsBreak = false;
    _enclosingBlockContainsContinue = false;
    try {
      final bodyExits = _nodeExits(node.body);
      final containsBreakOrContinue =
    ...
    try {
      ...
      if (forLoopParts is ForParts) {
        if (forLoopParts is ForPartsWithDeclarations) {
          ...
          final initialization = forLoopParts.initialization;
          if (initialization != null && _nodeExits(initialization)) {
            return true;
          }
        }
        final conditionExpression = forLoopParts.condition;
        ...
        final iterableExits = _nodeExits(forLoopParts.iterable);
  ...
  bool visitForStatement(ForStatement node) {
    ...
    try {
      if (parts is ForEachParts) {
        final iterableExits = _nodeExits(parts.iterable);
        // Discard whether the for-each body exits; since the for-each iterable
        // may be empty, execution may never enter the body, so it doesn't matter
        // if it exits or not.  We still must visit the body, to accurately
        // manage `_enclosingBlockBreaksLabel`.
  ...
  bool visitIfElement(IfElement node) {
    final conditionExpression = node.expression;
    ...
    final conditionValue = _knownConditionValue(conditionExpression);
    ...
    final thenExits = _nodeExits(thenElement);
    final elseExits = _nodeExits(elseElement);
    if (elseElement == null) {
      return false;
    }
  ...
  bool visitIfStatement(IfStatement node) {
    ...
    final thenExits = _nodeExits(thenStatement);
    final elseExits = _nodeExits(elseStatement);
  ...
  bool visitLabeledStatement(LabeledStatement node) {
    try {
      final statementExits = _nodeExits(node.statement);
      final neverBrokeFromLabel =
  ...
  bool visitMapLiteralEntry(MapLiteralEntry node) =>
      _nodeExits(node.key) || _nodeExits(node.value);
  ...
  @override
  bool visitSpreadElement(SpreadElement node) => _nodeExits(node.expression);
  ...
  bool visitSwitchExpressionCase(SwitchExpressionCase node) =>
      _nodeExits(node.guardedPattern.whenClause?.expression) ||
      _nodeExits(node.expression);
  ...
  bool visitSwitchStatement(SwitchStatement node) {
    ...
    try {
      ...
      var hasNonExitingCase = false;
      final List<SwitchMember> members = node.members;
      for (var i = 0; i < members.length; i++) {
        final switchMember = members[i];
  ...
  bool _visitVariableDeclarations(
    ...
    for (var i = variableDeclarations.length - 1; i >= 0; i--) {
  ...
  /// Return `true` if the given [node] exits.
  static bool exits(AstNode node) => ExitDetector()._nodeExits(node);
